...
const Probability = (data: event | undefined) => {
  ...
  useEffect(() => {
    if (data?.id) {
      ...
      if (svgRef.current) {
        ...
        const yCollision = d3
          .scaleLinear()
          .domain([0, d3.max(validData, (d) => d.collision_probability) || 0])
          .range([height, 0]);
        ...
        chart
          .append("g")
          .attr("class", "y-axis")
          .call(d3.axisLeft(yCollision).tickFormat(d3.format(".1e"))); // ".1e" keeps 1 decimal in scientific notation
  ...
  return (
    <div className={styles.container}>
      <h3>Collision Probability Over Time</h3>
      <svg ref={svgRef}></svg>
    </div>
  );
};
